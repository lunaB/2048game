...
int main(void)
{
	...
	gotoxy(13, 3); printf("■■■□■■■□□□■□■■■");
	gotoxy(10, 4); printf("┌────────────────┐");
	gotoxy(10, 5); printf("│  ▶   게임 시작                │");
	gotoxy(10, 6); printf("│       게임 종료                │");
	gotoxy(10, 7); printf("└────────────────┘");
	int ud = 0; //0이면위
	while (1) {
		input = _getch();
		...
		else if (ud == 1 && input == 13) {
			exit(0);
		}
		if (input == 0xe0 || input == 0) {  //특수키 입력받기
			input = _getch();
			if (input == UP) {
				ud = 0;
				gotoxy(14, 5); printf("▶");
	...
	for (turn = 0;;turn++)
	{
		while (1) {               //조건에 넣고싶은 공간에 값이 있다면 다시돔
			...
			if (map[rand1][rand2] == 0) {  //조건이 맞으면 break
				...
				if (rand3 == 1)      //1이생기면 2점*턴 추가
					score += 2*turn;
				else if (rand3 == 2)  //2가생기면 4점*턴 추가
					score += 4*turn;
				break;
			}
		}
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				...
				gotoxy(j * 10, (i * 4) + 1); printf("│%4d│", map[j][i]);
				gotoxy(j * 10, (i * 4) + 2); printf("└──┘");
			}
		}
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), WHITE);
		gotoxy(0, 16); printf("┌─────┬───────────┐");
		gotoxy(0, 17); printf("│2048 GAME │ score     %10d │", score);
		gotoxy(0, 18); printf("└─────┴───────────┘");
...
void gameover(int score)
{
	...
	gotoxy(0, 19); printf("└─────┴───────────┘");
	gotoxy(0, 20); printf("계속하시려면 엔터키를 눌러주세요.");
	scanf_s("");
	main();
}
...
void setcolor(int num) 
{
	...
	default :
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), WHITE); break;
